...
__global__ void gameOfLife(int* A, int* B) {
    ...
    if (row < n && col < n) {
        ...
        if (localRow > 0 && localCol > 0 && sharedA[localRow - 1][localCol - 1] == 1) {
            liveNeighbors++;  // Top-left
        }
        if (localRow > 0 && sharedA[localRow - 1][localCol]) {
            liveNeighbors++;  // Top
        }
        if (localRow > 0 && localCol < BLOCK_SIZE + 1 && sharedA[localRow - 1][localCol + 1]==1) {
            liveNeighbors++;  // Top-right
        }
        if (localCol > 0 && sharedA[localRow][localCol - 1] == 1) {
            liveNeighbors++;  // Left
        }
        if (localCol < BLOCK_SIZE + 1 && sharedA[localRow][localCol + 1] == 1) {
            liveNeighbors++;  // Right
        }
        if (localRow < BLOCK_SIZE + 1 && localCol > 0 && sharedA[localRow + 1][localCol - 1] == 1) {
            liveNeighbors++;  // Bottom-left
        }
        if (localRow < BLOCK_SIZE + 1 && sharedA[localRow + 1][localCol] == 1) {
            liveNeighbors++;  // Bottom
        }
        if (localRow < BLOCK_SIZE + 1 && localCol < BLOCK_SIZE + 1 && sharedA[localRow + 1][localCol + 1] == 1) {
            liveNeighbors++;  // Bottom-right
        }
